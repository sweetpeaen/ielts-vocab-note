...
- **标签分类**：预设科技/自然/教育/社会等类别标签，可新建、重命名、删除；按标签与熟练度筛选。录入表单中为**下拉多选**，可直接从已有标签勾选或在顶部「＋ 新建标签」。
- **熟练度**：熟练 / 中等 / 不熟，绿 / 黄 / 红三色印章标注。
- **句子库**：输入英文例句 →（可选）AI 返回中文示意 + 作文可用结构 + 固定搭配 + 另造 1~2 个新例句；不填 AI Key 也能**直接保存**句子，仅做本地笔记。
- **词组库**：与单词、句子并列的模块，收录英文词组/固定搭配 + 中文释义，支持「⚡ AI 填中文」自动释义（需配置 Key），同样可用标签分类与熟练度标记。
...
2. 到 **单词库** 点「＋ 新建」，输入单词（如 `abandon`），回车或点「查词并填充」→ 自动填好释义/词组/例句 → 选个标签和熟练度 → 保存。
3. 到 **词组库** 点「＋ 新建」，输入英文词组与中文释义（或点「⚡ AI 填中文」自动生成），选标签与熟练度后保存。
4. 到 **句子库**，贴一句英文例句，点「⚡ AI 分析并生成」或「直接保存」。
